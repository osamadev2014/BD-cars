import { validateDashboardAccess } from '@/lib/dashboard/validate'
import { DashboardPageHeader } from '@/components/dashboard/dashboard-page-header'
import { DashboardEmptyState } from '@/components/dashboard/dashboard-empty-state'
import { Container } from 'lucide-react'
import { notFound, redirect } from 'next/navigation'

interface Props {
  params: Promise<{ locale: string; orgType: string }>
}

export default async function VehicleCarriersPage({ params }: Props) {
  const { locale, orgType } = await params
  const isRtl = locale === 'ar'

  const validation = await validateDashboardAccess(locale, orgType)
  if (!validation.allowed) {
    if (validation.redirect) redirect(validation.redirect)
    notFound()
  }

  return (
    <div>
      <DashboardPageHeader
        title={isRtl ? 'ناقلات المركبات' : 'Vehicle Carriers'}
        breadcrumbs={[
          { label: isRtl ? 'لوحة التحكم' : 'Dashboard', href: `/${locale}/dashboard/${orgType}/overview` },
          { label: isRtl ? 'ناقلات المركبات' : 'Vehicle Carriers' },
        ]}
        locale={locale}
      />
      <DashboardEmptyState
        icon={<Container className="h-12 w-12" />}
        title={isRtl ? 'ناقلات المركبات' : 'Vehicle Carriers'}
        description={isRtl
          ? 'إدارة أسطول ناقلات المركبات'
          : 'Manage vehicle carrier fleet'}
      />
    </div>
  )
}
