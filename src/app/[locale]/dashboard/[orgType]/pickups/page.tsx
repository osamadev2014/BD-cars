...
export default async function PickupsPage({ params }: Props) {
  ...
  return (
    <div>
      <DashboardPageHeader
        title={isRtl ? 'عمليات الاستلام' : 'Pickup Operations'}
        breadcrumbs={[
          { label: isRtl ? 'لوحة التحكم' : 'Dashboard', href: `/${locale}/dashboard/${orgType}/overview` },
          { label: isRtl ? 'عمليات الاستلام' : 'Pickup Operations' },
        ]}
        locale={locale}
      />
      <DashboardEmptyState
        ...
        title={isRtl ? 'عمليات الاستلام' : 'Pickup Operations'}
        description={isRtl
          ? 'إدارة عمليات استلام المركبات'
          : 'Manage vehicle pickup operations'}
      />
    </div>
  )
}
