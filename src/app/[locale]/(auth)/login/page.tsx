    ...
    <div className="flex min-h-screen" dir="rtl">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative flex-col items-center justify-center bg-gradient-to-br from-primary via-primary/90 to-primary/70 p-12 overflow-hidden">
        ...
        <div className="relative z-10 text-center max-w-md">
          ...
          <p className="text-base text-white/70 max-w-sm mx-auto leading-relaxed">
            {locale === 'ar'
              ? 'بيع، شراء، وفحص سياراتك بكل ثقة وأمان'
              : 'Buy, sell, and inspect cars with confidence'}
          </p>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center px-5 sm:px-8 py-10 bg-background">
        <div className="w-full max-w-[400px] mx-auto">
          ...
          <h1 className="lg:hidden text-center text-2xl font-bold text-foreground mb-1">ريون</h1>
          <p className="lg:hidden text-center text-sm text-muted-foreground mb-8">
            {locale === 'ar' ? 'سوق السيارات الموثوق' : 'Trusted Auto Marketplace'}
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-0">
            ...
            <div className="mb-5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  maxLength={10}
                  className="block w-full h-12 rounded-xl border border-border bg-background text-foreground ps-[4.5rem] pe-3 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
                  autoComplete="tel"
                  dir="ltr"
                />
              </div>
            ...
            <label className="flex items-center gap-2 mb-6 cursor-pointer select-none">
              <div className="relative flex items-center justify-center">
                ...
                />
                <div className="w-5 h-5 rounded-md border-2 border-gray-300 peer-checked:border-primary peer-checked:bg-primary flex items-center justify-center transition-all">
                  {whatsapp && (
              ...
              <span className="text-xs text-muted-foreground leading-6">
                {locale === 'ar' ? 'أود استقبال التحديثات عبر واتساب' : 'I want to receive updates via WhatsApp'}
              </span>
            </label>
            ...
            <button
              ...
              className="w-full h-12 rounded-xl text-sm font-bold text-white bg-primary hover:bg-primary/90 disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow-sm"
            >
              {isLoading ? (
                ...
                locale === 'ar' ? 'إرسال رمز التحقق' : 'Send Code'
              )}
            </button>

            {/* Terms */}
            <div className="mt-5 text-center text-xs text-muted-foreground leading-relaxed">
              <div>{locale === 'ar' ? 'باستمرارك، توافق على' : 'By continuing, you agree to'}</div>
              <div>
                <Link href={`/${locale}/pages/terms`} className="text-muted-foreground hover:text-foreground underline underline-offset-2">
                  {locale === 'ar' ? 'الشروط والأحكام' : 'Terms & Conditions'}
                </Link>
                <span> {locale === 'ar' ? 'و' : ' & '} </span>
                <Link href={`/${locale}/pages/privacy`} className="text-muted-foreground hover:text-foreground underline underline-offset-2">
          ...
          <div className="mt-8 pt-6 border-t border-border/40">
            ...
            >
              ...
              {locale === 'ar' ? 'تسجيل دخول الشركات' : 'Business Login'}
            </Link>
          </div>
        </div>
      </div>
