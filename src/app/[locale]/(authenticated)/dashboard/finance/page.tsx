...
import { getFinanceRequests } from '@/lib/actions/finance-actions'
          ...
          return (
            <div key={req.id} className="rounded-lg border p-4 flex gap-4 items-start">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-muted-foreground">{t('price')}: {formatPrice(req.vehicle_price)}</p>
                <p className="text-sm text-muted-foreground">{t('down_payment')}: {formatPrice(req.down_payment || 0)}</p>
