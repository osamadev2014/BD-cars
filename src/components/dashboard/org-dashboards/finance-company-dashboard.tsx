...
import { Landmark, FileCheck, Clock, DollarSign } from 'lucide-react'
import { getFinanceCompanyStats } from '@/lib/actions/dashboard-actions'

interface Props { orgName: string; orgNameAr: string; locale: string; orgId: string }

const ICONS = [FileCheck, Clock, DollarSign, Landmark]
...
export function FinanceCompanyDashboard({ orgName, orgNameAr, locale, orgId }: Props) {
  const isRtl = locale === 'ar'
  const t = (en: string, ar: string) => isRtl ? ar : en
  const [data, setData] = useState<{ stats: any[]; recent: any[] }>({ stats: [], recent: [] })
  const [loading, setLoading] = useState(true)

  useEffect(() => { getFinanceCompanyStats(orgId).then((res) => { setData(res); setLoading(false) }) }, [orgId])

  if (loading) return <div className="flex justify-center py-12"><div className="animate-spin h-8 w-8 border-4 border-primary border-t-transparent rounded-full" /></div>

  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold">{isRtl ? orgNameAr : orgName}</h1><p className="text-muted-foreground">{t('Finance Dashboard', 'لوحة تحكم التمويل')}</p></div>
